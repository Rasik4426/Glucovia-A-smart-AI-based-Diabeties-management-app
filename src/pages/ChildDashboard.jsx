...
export default function ChildDashboard() {
  ...
  const { data: glucoseLogs = [] } = useQuery({
    ...
    queryFn: () => filterGlucoseLogs({ user_email: email }, '-log_date', 50),
    enabled: !!email,
  });

  const { data: mealLogs = [] } = useQuery({
    queryKey: ['mealLogs', email],
    queryFn: () => filterMealLogs({ user_email: email }, '-log_date', 20),
    enabled: !!email,
  });

  const { data: insulinLogs = [] } = useQuery({
    queryKey: ['insulinLogs', email],
    queryFn: () => filterInsulinLogs({ user_email: email }, '-log_date', 20),
    enabled: !!email,
  });
